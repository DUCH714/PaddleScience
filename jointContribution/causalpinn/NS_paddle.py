import numpy as np
import paddle
paddle.device.set_device("gpu:" + str(4))
...
class DataGenerator(Dataset):
    ...
    #@partial(paddle.jit.to_static, static_argnums=(0,))
...
class modified_MLP_II(paddle.nn.Layer):
    def __init__(self, layers, L_x=1.0, L_y=1.0, M_t=1, M_x=1, M_y=1, activation=nn.Tanh()):
        ...
        self.w_x = paddle.to_tensor(2.0 * np.pi / L_x)
        self.w_y = paddle.to_tensor(2.0 * np.pi / L_y)
        self.k_x = paddle.arange(1, M_x + 1)
        self.k_y = paddle.arange(1, M_y + 1)
        ...
        self.k_t = paddle.pow(paddle.to_tensor(10), paddle.arange(0, self.M_t + 1))

    def forward(self, inputs):
        t = inputs[:, 2:3]
        x = inputs[:, 0:1]
        y = inputs[:, 1:2]
        k_t = paddle.pow(paddle.to_tensor(10), paddle.arange(0, self.M_t + 1))[:, None]
...
class MLP(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        for W in self.Ws:
            inputs = self.activation(W(inputs))
        outputs = self.F(inputs)
        return outputs
...
class PINN((paddle.nn.Layer)):
    def __init__(self, w_exact, layers, M_t, M_x, M_y, state0, t0, t1, n_t, x_star, y_star, t_star, tol):
        ...
        self.network = modified_MLP_II(layers, L_x=2 * np.pi, L_y=2 * np.pi, M_t=M_t, M_x=M_x, M_y=M_y,
                                       activation=nn.Tanh())
        # self.network = MLP(layers, activation=nn.Tanh())
        ...
        scheduler = exponential_decay(1e-3, step_size=10000, gamma=0.9)
        self.optimizer = paddle.optimizer.Adam(learning_rate=scheduler,
                                               parameters=self.network.parameters())
    ...
    def residuals_and_weights(self, tol, batch):
        t_r, x_r, y_r = batch
        loss_u0, loss_v0, loss_w0 = self.loss_ics(self.x_star, self.y_star,
                                                  paddle.zeros_like(self.x_star).cuda())
    ...
    def compute_l2_error(self, x, y, t):
        ...
        w_pred = v_x - u_y
        l2_error = paddle.linalg.norm(
            w_pred - paddle.transpose(self.w_exact, (1, 2, 0)).reshape((-1, 1))) / paddle.linalg.norm(
            self.w_exact.reshape((-1, 1)))
        return l2_error

    def w_pred_fn(self, x, y, t):
        w_preds = []
        for tt in t:
            u_v = self.network(paddle.concat([x, y, tt * paddle.ones_like(x)], 1))
            u = u_v[:, 0:1]
            v = u_v[:, 1:2]
        ...
        w_preds = paddle.concat(w_preds, 0)
        return w_pred

    def output_u_v_w(self, x, y, t):
        u_v = self.network(paddle.concat([x, y, t * paddle.ones_like(x)], 1))
        u = u_v[:, 0:1]
        v = u_v[:, 1:2]
    ...
    def train(self, dataset, nIter=10000):
        ...
        for it in pbar:
            ...
            if it % 1000 == 0:
                l2_error_value = self.compute_l2_error(paddle.tile(self.x_star, (1, num_step)).reshape((-1, 1)),
                                                       paddle.tile(self.y_star, (1, num_step)).reshape((-1, 1)),
                                                       (paddle.tile(self.t_star[:num_step], (1, Nx ** 2)).T).reshape(
                                                           (-1, 1)))
                # print("ite:{},loss:{:.3e},error:{:.3e}".format(it,self.loss(batch).item(),l2_error_value.item()))
                lr=self.optimizer.get_lr()
                pbar.set_postfix({'ite': it, 'lr': lr*1000,'loss': self.loss(batch).item(), 'error': l2_error_value.item()})
...
for k in range(N):
    ...
    w_pred = model.w_pred_fn(x_star, y_star, t_star[:num_step])
    w_pred_list.append(w_pred)
    np.savez('paddle_w_pred_list.npz', w_pred_list=w_pred_list)

    u0_pred, v0_pred, w0_pred = model.output_u_v_w(x_star, y_star, t1)
    state0 = paddle.stack([u0_pred.reshape((128, 128)), v0_pred.reshape((128, 128)), w0_pred.reshape((128, 128))])
    state0.stop_gradient = True
